...
fn exec_resume_last_appends_to_existing_file() -> anyhow::Result<()> {
    ...
    Command::cargo_bin("tunacode-exec")
        .context("should find binary for tunacode-exec")?
        .env("tunacode_HOME", home.path())
    ...
    let mut binding = assert_cmd::Command::cargo_bin("tunacode-exec")
        .context("should find binary for tunacode-exec")?;
    let cmd = binding
        .env("tunacode_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
...
fn exec_resume_preserves_cli_configuration_overrides() -> anyhow::Result<()> {
    ...
    let output = Command::cargo_bin("tunacode-exec")
        .context("should find binary for tunacode-exec")?
        .env("tunacode_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("tunacode_RS_SSE_FIXTURE", &fixture)
