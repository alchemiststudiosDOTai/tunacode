...
use tunacode_core::protocol::McpInvocation;
...
use tunacode_exec::exec_events::AssistantMessageItem;
use tunacode_exec::exec_events::CommandExecutionItem;
use tunacode_exec::exec_events::CommandExecutionStatus;
use tunacode_exec::exec_events::ItemCompletedEvent;
use tunacode_exec::exec_events::ItemStartedEvent;
use tunacode_exec::exec_events::ItemUpdatedEvent;
use tunacode_exec::exec_events::McpToolCallItem;
use tunacode_exec::exec_events::McpToolCallStatus;
use tunacode_exec::exec_events::PatchApplyStatus;
use tunacode_exec::exec_events::PatchChangeKind;
use tunacode_exec::exec_events::ReasoningItem;
use tunacode_exec::exec_events::ThreadErrorEvent;
use tunacode_exec::exec_events::ThreadEvent;
use tunacode_exec::exec_events::ThreadItem;
use tunacode_exec::exec_events::ThreadItemDetails;
use tunacode_exec::exec_events::ThreadStartedEvent;
use tunacode_exec::exec_events::TodoItem as ExecTodoItem;
use tunacode_exec::exec_events::TodoListItem as ExecTodoListItem;
use tunacode_exec::exec_events::TurnCompletedEvent;
use tunacode_exec::exec_events::TurnFailedEvent;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_id = tunacode_protocol::mcp_protocol::ConversationId::from_string(
    ...
    let ev = event(
        ...
        EventMsg::SessionConfigured(SessionConfiguredEvent {
            session_id,
            model: "tunacode-mini-latest".to_string(),
...
fn task_started_produces_turn_started_event() {
    ...
    let out = ep.collect_thread_events(&event(
        "t1",
        EventMsg::TaskStarted(tunacode_core::protocol::TaskStartedEvent {
...
fn plan_update_emits_todo_list_started_updated_and_completed() {
    ...
    let complete = event(
        "p3",
        EventMsg::TaskComplete(tunacode_core::protocol::TaskCompleteEvent {
...
fn error_event_produces_error() {
    ...
    let out = ep.collect_thread_events(&event(
        "e1",
        EventMsg::Error(tunacode_core::protocol::ErrorEvent {
...
fn stream_error_event_produces_error() {
    ...
    let out = ep.collect_thread_events(&event(
        "e1",
        EventMsg::StreamError(tunacode_core::protocol::StreamErrorEvent {
...
fn task_complete_produces_turn_completed_with_usage() {
    ...
    let usage = tunacode_core::protocol::TokenUsage {
        input_tokens: 1200,
    ...
    let info = tunacode_core::protocol::TokenUsageInfo {
    ...
    let token_count_event = event(
        "e1",
        EventMsg::TokenCount(tunacode_core::protocol::TokenCountEvent {
            info: Some(info),
            rate_limits: None,
        }),
    );
