...
pub async fn run_main(cli: Cli, tunacode_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let prompt = match prompt_arg {
        ...
        maybe_dash => {
            ...
            // writing to stdin, tunacode will hang indefinitely, so this should
    ...
    let conversation_manager =
        ConversationManager::new(AuthManager::shared(config.tunacode_home.clone()));
        ...
        if let Some(path) = resume_path {
            conversation_manager
                .resume_conversation_from_rollout(
                    config.clone(),
                    path,
                    AuthManager::shared(config.tunacode_home.clone()),
                )
                .await?
        } else {
            conversation_manager
    ...
    // Print the effective configuration and prompt so users can see what tunacode
    // is using.
    event_processor.print_config_summary(&config, &prompt, &session_configured);

    info!("tunacode initialized with event: {session_configured:?}");
            ...
            loop {
                tokio::select! {
                    _ = tokio::signal::ctrl_c() => {
                        ...
                        // Immediately notify tunacode to abort any in‑flight task.
                        conversation.submit(Op::Interrupt).await.ok();

                        // Exit the inner loop and return to the main input prompt. The tunacode
    ...
    while let Some(event) = rx.recv().await {
        ...
        let shutdown: TunacodeStatus = event_processor.process_event(event);
        match shutdown {
            TunacodeStatus::Running => continue,
            TunacodeStatus::InitiateShutdown => {
                conversation.submit(Op::Shutdown).await?;
            }
            TunacodeStatus::Shutdown => {
                break;
            }
        }
    }
...
async fn resolve_resume_path(
    ...
    if args.last {
        ...
        let path = find_conversation_path_by_id_str(&config.tunacode_home, id_str).await?;
        Ok(path)
    } else {
        Ok(None)
    }
