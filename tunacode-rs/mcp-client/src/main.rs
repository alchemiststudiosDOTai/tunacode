...
//!
//! ```bash
//! cargo run -p tunacode-mcp-client -- `tunacode-mcp-server`
//! ```
//!
...
async fn main() -> Result<()> {
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "tunacode-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("tunacode".to_string()),
            // This field is used by tunacode when it is an MCP server: it should
            // not be used when tunacode is an MCP client.
            user_agent: None,
        },
