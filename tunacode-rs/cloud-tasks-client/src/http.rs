...
use tunacode_backend_client as backend;
use tunacode_backend_client::CodeTaskDetailsResponseExt;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn messages(&self, id: TaskId) -> Result<Vec<String>> {
            ...
            let url = match details_path(self.base_url, &id.0) {
                Some(url) => url,
                None => format!("{}/api/tunacode/tasks/{}", self.base_url, id.0),
            };
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("tunacode_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
    ...
    impl<'a> Apply<'a> {
        ...
        pub(crate) async fn run(
            ...
            let req = tunacode_git_apply::ApplyGitRequest {
                cwd: std::env::current_dir().unwrap_or_else(|_| std::env::temp_dir()),
                diff: diff.clone(),
                revert: false,
                preflight,
            };
            let r = tunacode_git_apply::apply_git_patch(&req)
    ...
    fn details_path(base_url: &str, id: &str) -> Option<String> {
        ...
        } else if base_url.contains("/api/tunacode") {
