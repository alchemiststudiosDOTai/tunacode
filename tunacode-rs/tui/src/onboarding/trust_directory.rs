...
impl WidgetRef for &TrustDirectoryWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "You are running tunacode in ".bold(),
        ...
        if self.is_git_repo {
            lines.push(
                "  Since this folder is version controlled, you may wish to allow tunacode".into(),
            );
        ...
        if self.is_git_repo {
            lines.push(create_option(
                ...
                "Yes, allow tunacode to work in this folder without asking for approval",
            ));
            lines.push(create_option(
                1,
            ...
            lines.push(create_option(
                0,
                TrustDirectorySelection::Trust,
                "Allow tunacode to work in this folder without asking for approval",
            ));
            lines.push(create_option(
                1,
                TrustDirectorySelection::DontTrust,
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        ...
        if let Err(e) = set_project_trusted(&self.tunacode_home, &target) {
