...
impl App {
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<bool> {
        match event {
            ...
            AppEvent::TunacodeEvent(event) => {
                self.chat_widget.handle_tunacode_event(event);
            }
            ...
            AppEvent::TunacodeOp(op) => self.chat_widget.submit_op(op),
            ...
            AppEvent::PersistModelSelection { model, effort } => {
                ...
                match persist_model_selection(&self.config.tunacode_home, profile, &model, effort)
                    .await
                {
                    Ok(()) => {
    ...
    pub(crate) fn token_usage(&self) -> tunacode_core::protocol::TokenUsage {
...
mod tests {
    ...
    fn make_test_app() -> App {
        ...
        let server = Arc::new(ConversationManager::with_auth(tunacodeAuth::from_api_key(
            "Test API Key",
        )));
        let auth_manager =
            AuthManager::from_auth_for_testing(tunacodeAuth::from_api_key("Test API Key"));
        let file_search = FileSearchManager::new(config.cwd.clone(), app_event_tx.clone());
