...
use tunacode_core::config_types::ReasoningSummaryFormat;
...
impl HistoryCell for AgentMessageCell {
    ...
    fn transcript_lines(&self) -> Vec<Line<'static>> {
        ...
        if self.is_first_line {
            out.push("tunacode".magenta().bold().into());
        }
...
pub(crate) fn new_session_info(
    ...
    if is_first_event {
        ...
        let header = SessionHeaderHistoryCell::new(
            ...
            reasoning_effort,
            config.cwd.clone(),
            crate::version::TUNACODE_CLI_VERSION,
        );
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for tunacode".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/approvals".into(),
                " - choose what tunacode can do without approval".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ OpenAI tunacode (vX)"
        let title_spans: Vec<Span<'static>> = vec![
            Span::from(">_ ").dim(),
            Span::from("OpenAI tunacode").bold(),
            Span::from(" ").dim(),
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://github.com/openai/tunacode/blob/main/docs/config.md#mcp_servers\u{7}MCP docs\u{1b}]8;;\u{7}".underlined(),
