...
// The standalone `tunacode-tui` binary prints a short help message before the
...
use tunacode_core::config::find_tunacode_home;
use tunacode_core::config::load_config_as_toml_with_cli_overrides;
...
pub async fn run_main(
    ...
    let overrides_cli = tunacode_common::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let config_toml = {
        let tunacode_home = match find_tunacode_home() {
            Ok(tunacode_home) => tunacode_home,
            Err(err) => {
                eprintln!("Error finding tunacode home: {err}");
                std::process::exit(1);
            }
        };

        match load_config_as_toml_with_cli_overrides(&tunacode_home, cli_kv_overrides) {
    ...
    let log_dir = tunacode_core::config::log_dir(&config)?;
    ...
    // tunacode CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("tunacode-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for tunacode crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("tunacode_core=info,tunacode_tui=info,tunacode_rmcp_client=info")
        })
    };
    ...
    if cli.oss {
        tunacode_ollama::ensure_oss_ready(&config)
            .await
            .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
    }

    let otel = tunacode_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"));
    ...
    if let Some(provider) = otel.as_ref() {
        let otel_layer = OpenTelemetryTracingBridge::new(&provider.logger).with_filter(
            tracing_subscriber::filter::filter_fn(tunacode_core::otel_init::tunacode_export_filter),
        );
...
async fn run_ratatui_app(
    ...
    if let Some(latest_version) = updates::get_upgrade_version(&config) {
        ...
        let managed_by_npm = std::env::var_os("tunacode_MANAGED_BY_NPM").is_some();
        ...
        let mut content_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(""),
            Line::from(
                "https://github.com/openai/tunacode/releases/latest"
                    .cyan()
                    .underlined(),
            ),
            Line::from(""),
        ...
        if managed_by_npm {
            let npm_cmd = "npm install -g @openai/tunacode@latest";
        ...
        {
            let brew_cmd = "brew upgrade tunacode";
            content_lines.push(Line::from(vec![
                "Run ".into(),
                brew_cmd.cyan(),
                " to update.".into(),
            ...
            content_lines.push(Line::from(vec![
                "See ".into(),
                "https://github.com/openai/tunacode".cyan().underlined(),
    ...
    let resume_selection = if let Some(id_str) = cli.resume_session_id.as_deref() {
        match find_conversation_path_by_id_str(&config.tunacode_home, id_str).await? {
        ...
        match RolloutRecorder::list_conversations(&config.tunacode_home, 1, None).await {
            Ok(page) => page
                .items
                .first()
        ...
        match resume_picker::run_resume_picker(&mut tui, &config.tunacode_home).await? {
            resume_picker::ResumeSelection::Exit => {
                restore();
                session_log::log_session_end();
                return Ok(AppExitInfo {
                    token_usage: tunacode_core::protocol::TokenUsage::default(),
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let tunacode_home = config.tunacode_home.clone();
        match tunacodeAuth::from_tunacode_home(&tunacode_home) {
