...
use tunacode_core::plan_tool::PlanItemArg;
...
use tunacode_core::protocol::PatchApplyEndEvent;
use tunacode_core::protocol::ReviewCodeLocation;
use tunacode_core::protocol::ReviewFinding;
use tunacode_core::protocol::ReviewLineRange;
use tunacode_core::protocol::ReviewOutputEvent;
...
fn final_answer_without_newline_is_flushed_immediately() {
    ...
    chat.handle_tunacode_event(Event {
        id: "sub-a".into(),
        msg: EventMsg::AgentMessageDelta(AgentMessageDeltaEvent {
            delta: "Hi! How can I help with tunacode-rs or anything else today?".into(),
        }),
    });
    ...
    chat.handle_tunacode_event(Event {
        ...
        msg: EventMsg::AgentMessage(AgentMessageEvent {
            message: "Hi! How can I help with tunacode-rs or anything else today?".into(),
        }),
    });
    ...
    assert!(
        ...
        "expected 'tunacode' header to be emitted",
    );
    let found_final = cells.iter().any(|lines| {
        let s = lines
            ...
            .map(|sp| sp.content.clone())
            .collect::<String>();
        s.contains("Hi! How can I help with tunacode-rs or anything else today?")
    });
    assert!(
        found_final,
...
fn resumed_initial_messages_render_history() {
    ...
    let configured = tunacode_core::protocol::SessionConfiguredEvent {
...
fn make_chatwidget_manual() -> (
    ...
    let bottom = BottomPane::new(BottomPaneParams {
        ...
        enhanced_keys_supported: false,
        placeholder_text: "Ask tunacode to do anything".to_string(),
        disable_paste_burst: false,
    });
    let auth_manager = AuthManager::from_auth_for_testing(tunacodeAuth::from_api_key("test"));
    let widget = ChatWidget {
        app_event_tx,
        tunacode_op_tx: op_tx,
        bottom_pane: bottom,
        active_cell: None,
        config: cfg.clone(),
        auth_manager,
...
fn begin_exec(chat: &mut ChatWidget, call_id: &str, raw_cmd: &str) {
    ...
    let parsed_cmd: Vec<ParsedCommand> = tunacode_core::parse_command::parse_command(&command)
        .into_iter()
        .map(Into::into)
        .collect();
    chat.handle_tunacode_event(Event {
...
fn review_commit_picker_shows_subjects_without_timestamps() {
    ...
    let entries = vec![
        tunacode_core::git_info::CommitLogEntry {
...
fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::TunacodeOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::TunacodeOp(Op::Review { review_request }) => {
            assert_eq!(
...
fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::tunacodeOp should be sent
...
fn interrupt_exec_marks_failed_snapshot() {
    ...
    chat.handle_tunacode_event(Event {
        id: "call-int".into(),
        msg: EventMsg::TurnAborted(tunacode_core::protocol::TurnAbortedEvent {
...
//
// Synthesizes a tunacode ExecApprovalRequest event to trigger the approval modal
...
fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    // Expect a tunacodeOp with PatchApproval carrying the submission id, not call id
    let mut found = false;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::TunacodeOp(Op::PatchApproval { id, decision }) = app_ev {
            assert_eq!(id, "sub-123");
            assert!(matches!(
                decision,
                tunacode_core::protocol::ReviewDecision::Approved
            ));
            found = true;
            break;
        }
...
fn apply_patch_full_flow_integration_like() {
    ...
    // 2) User approves via 'y' and App receives a tunacodeOp
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE));
    let mut maybe_op: Option<Op> = None;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::TunacodeOp(op) = app_ev {
            maybe_op = Some(op);
            break;
        }
    }
    let op = maybe_op.expect("expected tunacodeOp after key press");

    // 3) App forwards to widget.submit_op, which pushes onto tunacode_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to tunacode channel");
    match forwarded {
...
fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    chat.handle_tunacode_event(Event {
        ...
        msg: EventMsg::ExecCommandBegin(ExecCommandBeginEvent {
            ...
            cwd: std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")),
            parsed_cmd: vec![
                tunacode_core::parse_command::ParsedCommand::Search {
                    query: Some("Change Approved".into()),
                    path: None,
                    cmd: "rg \"Change Approved\"".into(),
                }
                .into(),
                tunacode_core::parse_command::ParsedCommand::Read {
