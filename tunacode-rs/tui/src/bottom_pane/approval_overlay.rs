...
mod tests {
    ...
    fn shortcut_triggers_selection() {
        ...
        // We expect at least one tunacodeOp message in the queue.
        let mut saw_op = false;
        while let Ok(ev) = rx.try_recv() {
            if matches!(ev, AppEvent::TunacodeOp(_)) {
                saw_op = true;
                break;
            }
        }
    ...
    fn enter_sets_last_selected_index_without_dismissing() {
        ...
        while let Ok(ev) = rx.try_recv() {
            if let AppEvent::TunacodeOp(Op::ExecApproval { decision: d, .. }) = ev {
                decision = Some(d);
                break;
            }
        }
