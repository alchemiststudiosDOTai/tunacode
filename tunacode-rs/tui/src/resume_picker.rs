...
pub async fn run_resume_picker(tui: &mut Tui, tunacode_home: &Path) -> Result<ResumeSelection> {
...
impl PickerState {
    fn new(tunacode_home: PathBuf, requester: FrameRequester, page_loader: PageLoader) -> Self {
        Self {
            tunacode_home,
            requester,
            pagination: PaginationState {
    ...
    fn load_more_if_needed(&mut self, trigger: LoadTrigger) {
        ...
        (self.page_loader)(PageLoadRequest {
            tunacode_home: self.tunacode_home.clone(),
            cursor: Some(cursor),
            request_token,
            search_token,
        });
