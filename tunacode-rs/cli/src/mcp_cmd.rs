...
use tunacode_core::config::write_global_mcp_servers;
use tunacode_core::config_types::McpServerConfig;
use tunacode_core::config_types::McpServerTransportConfig;

/// [experimental] Launch tunacode as an MCP server or manage configured MCP servers.
///
/// Subcommands:
/// - `serve`  — run the MCP server on stdio
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.tunacode/config.toml`
...
fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `tunacode mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: tunacode mcp remove {}", get_args.name);
