...
fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = tunacode_command(tunacode_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = tunacode_command(tunacode_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = tunacode_command(tunacode_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
