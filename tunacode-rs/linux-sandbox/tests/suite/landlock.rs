...
async fn run_cmd(cmd: &[&str], writable_roots: &[PathBuf], timeout_ms: u64) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_tunacode-linux-sandbox");
    let tunacode_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_tunacode-linux-sandbox");
    let tunacode_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
    ...
    let output = match result {
        Ok(output) => output,
        Err(tunacodeErr::Sandbox(SandboxErr::Denied { output })) => *output,
        _ => {
