...
const DEFAULT_COMMIT_MESSAGE: &str = "tunacode snapshot";
...
pub fn create_ghost_commit(
    ...
    let index_tempdir = Builder::new().prefix("tunacode-git-index-").tempdir()?;
...
fn default_commit_identity() -> Vec<(OsString, OsString)> {
    vec![
        (
            ...
            OsString::from("tunacode Snapshot"),
        ),
        (
            OsString::from("GIT_AUTHOR_EMAIL"),
            OsString::from("snapshot@tunacode.local"),
        ),
        (
...
mod tests {
    ...
    fn restore_from_subdirectory_restores_files_relatively() -> Result<(), GitToolingError> {
        ...
        assert!(!workspace.join("tunacode-rs").exists());
    ...
    fn restore_from_subdirectory_preserves_parent_vscode() -> Result<(), GitToolingError> {
        ...
        let workspace = repo.join("tunacode-rs");
