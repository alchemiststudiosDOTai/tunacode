...
use tunacode_mcp_server::ExecApprovalElicitRequestParams;
use tunacode_mcp_server::ExecApprovalResponse;
use tunacode_mcp_server::PatchApprovalElicitRequestParams;
use tunacode_mcp_server::PatchApprovalResponse;
use tunacode_mcp_server::TunacodeToolCallParam;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(TUNACODE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a tunacode sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "tunacode" tool request, which should hit the completions endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let tunacode_request_id = mcp_process
        .send_tunacode_tool_call(TunacodeToolCallParam {
    ...
    let expected_elicitation_request = create_expected_elicitation_request(
        ...
        tunacode_request_id.to_string(),
        params.tunacode_event_id.clone(),
    )?;
    ...
    // Verify the original `tunacode` tool call completes and that the file was created.
    let tunacode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(tunacode_request_id)),
    )
    .await??;
    assert_eq!(
        JSONRPCResponse {
            jsonrpc: JSONRPC_VERSION.into(),
            id: RequestId::Integer(tunacode_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request(
    ...
    command: Vec<String>,
    workdir: &Path,
    tunacode_mcp_tool_call_id: String,
    tunacode_event_id: String,
) -> anyhow::Result<JSONRPCRequest> {
    let expected_message = format!(
        "Allow tunacode to run `{}` in `{}`?",
    ...
    Ok(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(&ExecApprovalElicitRequestParams {
            ...
            tunacode_elicitation: "exec-approval".to_string(),
            tunacode_mcp_tool_call_id,
            tunacode_event_id,
            tunacode_command: command,
            tunacode_cwd: workdir.to_path_buf(),
            tunacode_call_id: "call1234".to_string(),
        })?),
    })
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "tunacode" tool request that will trigger the apply_patch command
    let tunacode_request_id = mcp_process
        .send_tunacode_tool_call(TunacodeToolCallParam {
    ...
    // Verify the original `tunacode` tool call completes
    let tunacode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(tunacode_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_tunacode_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = tunacode_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn tunacode_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `tunacode mcp` with a specific config.toml.
    let tunacode_home = TempDir::new()?;
    create_config_toml(tunacode_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(tunacode_home.path()).await?;
...
fn create_expected_patch_approval_elicitation_request(
    ...
    message_lines.push("Allow tunacode to apply proposed code changes?".to_string());
    ...
    Ok(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(&PatchApprovalElicitRequestParams {
            ...
            tunacode_elicitation: "patch-approval".to_string(),
            tunacode_mcp_tool_call_id,
            tunacode_event_id,
            tunacode_reason: reason,
            tunacode_grant_root: grant_root,
            tunacode_changes: changes,
            tunacode_call_id: "call1234".to_string(),
        })?),
    })
}
...
/// Create a tunacode config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(tunacode_home: &Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = tunacode_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
