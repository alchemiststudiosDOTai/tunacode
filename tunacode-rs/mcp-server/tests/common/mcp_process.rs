...
impl McpProcess {
    pub async fn new(tunacode_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(tunacode_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let std_cmd = StdCommand::cargo_bin("tunacode-mcp-server")
            .context("should find binary for tunacode-mcp-server")?;
        ...
        cmd.env("tunacode_HOME", tunacode_home);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("tunacode-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let user_agent = format!(
            "tunacode_cli_rs/0.0.0 ({} {}; {}) {} (elicitation test; 0.0.0)",
            os_info.os_type(),
            os_info.version(),
            os_info.architecture().unwrap_or("unknown"),
            tunacode_core::terminal::user_agent()
        );
        assert_eq!(
    ...
    pub async fn send_tunacode_tool_call(
        &mut self,
        params: TunacodeToolCallParam,
    ) -> anyhow::Result<i64> {
        let tunacode_tool_call_params = CallToolRequestParams {
            name: "tunacode".to_string(),
            arguments: Some(serde_json::to_value(params)?),
        };
        self.send_request(
            mcp_types::CallToolRequest::METHOD,
            Some(serde_json::to_value(tunacode_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "tunacode/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JSONRPCMessage::Notification(notification) => {
                    let is_match = if notification.method == "tunacode/event" {
