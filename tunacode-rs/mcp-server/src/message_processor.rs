...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_tunacode_uuid: Arc<Mutex<HashMap<RequestId, ConversationId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            running_requests_id_to_tunacode_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let result = mcp_types::InitializeResult {
            ...
            server_info: mcp_types::Implementation {
                name: "tunacode-mcp-server".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
                title: Some("tunacode".to_string()),
                user_agent: Some(get_tunacode_user_agent()),
            },
        };
    ...
    async fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![
                create_tool_for_tunacode_tool_call_param(),
                create_tool_for_tunacode_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(
        ...
        match name.as_str() {
            "tunacode" => self.handle_tool_call_tunacode(id, arguments).await,
            "tunacode-reply" => {
                self.handle_tool_call_tunacode_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_tunacode(&self, id: RequestId, arguments: Option<serde_json::Value>) {
                    ...
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![ContentBlock::TextContent(TextContent {
                                ...
                                text: format!(
                                    "Failed to load tunacode configuration from overrides: {e}"
                                ),
                                annotations: None,
                            })],
                            is_error: Some(true),
            ...
            None => {
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_string(),
                        text:
                            "Missing arguments for tunacode tool-call; the `prompt` field is required."
                                .to_string(),
                        annotations: None,
                    })],
                    is_error: Some(true),
        ...
        task::spawn(async move {
            // Run the tunacode session and stream events back to the client.
            crate::tunacode_tool_runner::run_tunacode_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_tunacode_session_reply(
        &self,
        ...
        // parse arguments
        let TunacodeToolCallReplyParam {
            conversation_id,
            prompt,
        } = match arguments {
            Some(json_val) => {
            ...
            None => {
                ...
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_owned(),
                        text: "Missing arguments for tunacode-reply tool-call; the `conversation_id` and `prompt` fields are required.".to_owned(),
                        annotations: None,
                    })],
                    is_error: Some(true),
                    structured_content: None,
        ...
        tokio::spawn({
            ...
            let running_requests_id_to_tunacode_uuid = running_requests_id_to_tunacode_uuid.clone();

            async move {
                crate::tunacode_tool_runner::run_tunacode_tool_session_reply(
                    tunacode,
                    outgoing,
                    request_id,
                    prompt,
    ...
    async fn handle_cancelled_notification(
        ...
        let conversation_id = {
            let map_guard = self.running_requests_id_to_tunacode_uuid.lock().await;
        ...
        // Obtain the tunacode conversation from the server.
        let tunacode_arc = match self
            .conversation_manager
            .get_conversation(conversation_id)
            .await
        {
        ...
        // Submit interrupt to tunacode.
        let err = tunacode_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: tunacode_core::protocol::Op::Interrupt,
            })
            .await;
        if let Err(e) = err {
            tracing::error!("Failed to submit interrupt to tunacode: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_tunacode_uuid
            .lock()
            .await
            .remove(&request_id);
    }
