...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the tunacode tool call.
    ...
    pub tunacode_command: Vec<String>,
    pub tunacode_cwd: PathBuf,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    let message = format!(
        "Allow tunacode to run `{escaped_command}` in `{cwd}`?",
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_exec_approval_response(event_id, on_response, tunacode).await;
        });
    }
}
