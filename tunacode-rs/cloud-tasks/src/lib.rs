...
struct ApplyJob {
    task_id: tunacode_cloud_tasks_client::TaskId,
    diff_override: Option<String>,
}

fn level_from_status(status: tunacode_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        tunacode_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        tunacode_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        tunacode_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}

fn spawn_preflight(
    app: &mut app::App,
    backend: &Arc<dyn tunacode_cloud_tasks_client::CloudBackend>,
    ...
    tokio::spawn(async move {
        ...
        let result = tunacode_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = tunacode_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
pub async fn run_main(
    ...
    set_user_agent_suffix("tunacode_cloud_tasks_tui");

    // Default to online unless explicitly configured to use mock.
    let use_mock = matches!(
        std::env::var("tunacode_CLOUD_TASKS_MODE").ok().as_deref(),
        Some("mock") | Some("MOCK")
    );

    let backend: Arc<dyn tunacode_cloud_tasks_client::CloudBackend> = if use_mock {
        Arc::new(tunacode_cloud_tasks_client::MockClient)
    } else {
        // Build an HTTP client against the configured (or default) base URL.
        let base_url = std::env::var("tunacode_CLOUD_TASKS_BASE_URL")
            .unwrap_or_else(|_| "https://chatgpt.com/backend-api".to_string());
        let ua = tunacode_core::default_client::get_tunacode_user_agent();
        let mut http =
            tunacode_cloud_tasks_client::HttpClient::new(base_url.clone())?.with_user_agent(ua);
        ...
        let _token = match tunacode_core::config::find_tunacode_home()
            .ok()
            .map(tunacode_login::AuthManager::new)
            .and_then(|am| am.auth())
        {
            Some(auth) => {
                ...
                match auth.get_token().await {
                    ...
                    _ => {
                        eprintln!(
                            "Not signed in. Please run 'tunacode login' to sign in with ChatGPT, then re-run 'tunacode cloud'."
                        );
                        std::process::exit(1);
                    }
                }
    ...
    let force_internal = matches!(
        std::env::var("tunacode_CLOUD_TASKS_FORCE_INTERNAL")
            .ok()
            .as_deref(),
    ...
    append_error_log(format!(
        ...
        force_internal,
        tunacode_core::default_client::get_tunacode_user_agent()
    ));
    ...
    // Fetch environment list in parallel so the header can show friendly names quickly.
    {
        ...
        tokio::spawn(async move {
            let base_url = util::normalize_base_url(
                &std::env::var("tunacode_CLOUD_TASKS_BASE_URL")
                    .unwrap_or_else(|_| "https://chatgpt.com/backend-api".to_string()),
            );
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx.send(Instant::now() + tunacode_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match tunacode_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        app::AppEvent::ApplyFinished { id, result } => {
                            ...
                            match result {
                                Ok(outcome) => {
                                    ...
                                    if matches!(outcome.status, tunacode_cloud_tasks_client::ApplyStatus::Success) {
                    ...
                    Some(Ok(Event::Key(key))) if matches!(key.kind, KeyEventKind::Press | KeyEventKind::Repeat) => {
                        ...
                        if is_ctrl_o && app.new_task.is_some() {
                            ...
                            if should_fetch {
                                    let tx = tx.clone();
                                    tokio::spawn(async move {
            let base_url = crate::util::normalize_base_url(&std::env::var("tunacode_CLOUD_TASKS_BASE_URL").unwrap_or_else(|_| "https://chatgpt.com/backend-api".to_string()));
            let headers = crate::util::build_chatgpt_headers().await;
                                        let res = crate::env_detect::list_environments(&base_url, &headers).await;
                                        let _ = tx.send(app::AppEvent::EnvironmentsLoaded(res));
                                    });
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                    if page.submitting {
                                        // Ignore input while submitting
                                    } else if let tunacode_tui::ComposerAction::Submitted(text) = page.composer.input(key) {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    let result = tunacode_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, "main", false, best_of_n).await;
            ...
            let headers = crate::util::build_chatgpt_headers().await;
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match tunacode_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match tunacode_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = tunacode_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match tunacode_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
