...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = tunacode_core::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("tunacode-cli")),
    );
    if let Ok(home) = tunacode_core::config::find_tunacode_home() {
        let am = tunacode_login::AuthManager::new(home);
