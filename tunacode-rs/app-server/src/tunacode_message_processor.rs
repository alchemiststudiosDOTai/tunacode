...
use tunacode_core::config::load_config_as_toml;
use tunacode_core::config_edit::CONFIG_KEY_EFFORT;
use tunacode_core::config_edit::CONFIG_KEY_MODEL;
use tunacode_core::config_edit::persist_overrides_and_clear_if_none;
...
use tunacode_core::git_info::git_diff_to_remote;
...
use tunacode_core::protocol::ExecApprovalRequestEvent;
...
use tunacode_protocol::mcp_protocol::APPLY_PATCH_APPROVAL_METHOD;
...
use tunacode_protocol::mcp_protocol::ApplyPatchApprovalParams;
use tunacode_protocol::mcp_protocol::ApplyPatchApprovalResponse;
...
use tunacode_protocol::mcp_protocol::AuthStatusChangeNotification;
use tunacode_protocol::mcp_protocol::ClientRequest;
use tunacode_protocol::mcp_protocol::ConversationId;
use tunacode_protocol::mcp_protocol::ConversationSummary;
use tunacode_protocol::mcp_protocol::EXEC_COMMAND_APPROVAL_METHOD;
use tunacode_protocol::mcp_protocol::ExecArbitraryCommandResponse;
use tunacode_protocol::mcp_protocol::ExecCommandApprovalParams;
use tunacode_protocol::mcp_protocol::ExecCommandApprovalResponse;
use tunacode_protocol::mcp_protocol::ExecOneOffCommandParams;
use tunacode_protocol::mcp_protocol::FuzzyFileSearchParams;
use tunacode_protocol::mcp_protocol::FuzzyFileSearchResponse;
use tunacode_protocol::mcp_protocol::GetUserAgentResponse;
use tunacode_protocol::mcp_protocol::GetUserSavedConfigResponse;
use tunacode_protocol::mcp_protocol::GitDiffToRemoteResponse;
use tunacode_protocol::mcp_protocol::InputItem as WireInputItem;
use tunacode_protocol::mcp_protocol::InterruptConversationParams;
use tunacode_protocol::mcp_protocol::InterruptConversationResponse;
...
use tunacode_protocol::mcp_protocol::RemoveConversationListenerParams;
use tunacode_protocol::mcp_protocol::RemoveConversationSubscriptionResponse;
...
use tunacode_protocol::mcp_protocol::SendUserTurnParams;
use tunacode_protocol::mcp_protocol::SendUserTurnResponse;
...
/// Handles JSON-RPC messages for tunacode conversations.
pub(crate) struct TunacodeMessageProcessor {
...
impl TunacodeMessageProcessor {
    ...
    async fn login_api_key(&mut self, request_id: RequestId, params: LoginApiKeyParams) {
        ...
        match login_with_api_key(&self.config.tunacode_home, &params.api_key) {
            Ok(()) => {
    ...
    async fn login_chatgpt(&mut self, request_id: RequestId) {
        ...
        let opts = LoginServerOptions {
            open_browser: false,
            ..LoginServerOptions::new(config.tunacode_home.clone(), CLIENT_ID.to_string())
        };
    ...
    async fn cancel_login_chatgpt(&mut self, request_id: RequestId, login_id: Uuid) {
        ...
        if guard.as_ref().map(|l| l.login_id) == Some(login_id) {
            ...
            self.outgoing
                .send_response(
                    request_id,
                    tunacode_protocol::mcp_protocol::CancelLoginChatGptResponse {},
                )
                .await;
        } else {
            drop(guard);
    ...
    async fn logout_chatgpt(&mut self, request_id: RequestId) {
        ...
        self.outgoing
            .send_response(
                request_id,
                tunacode_protocol::mcp_protocol::LogoutChatGptResponse {},
            )
            .await;
    ...
    async fn get_auth_status(
        ...
        params: tunacode_protocol::mcp_protocol::GetAuthStatusParams,
    ) {
        ...
        let response = if !requires_openai_auth {
            tunacode_protocol::mcp_protocol::GetAuthStatusResponse {
                auth_method: None,
                auth_token: None,
            ...
            match self.auth_manager.auth() {
                ...
                None => tunacode_protocol::mcp_protocol::GetAuthStatusResponse {
                    auth_method: None,
                    auth_token: None,
                    requires_openai_auth: Some(true),
                },
    ...
    async fn get_user_saved_config(&self, request_id: RequestId) {
        let toml_value = match load_config_as_toml(&self.config.tunacode_home) {
            Ok(val) => val,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INTERNAL_ERROR_CODE,
    ...
    async fn get_user_info(&self, request_id: RequestId) {
        ...
        let auth_path = get_auth_file(&self.config.tunacode_home);
    ...
    async fn exec_one_off_command(&self, request_id: RequestId, params: ExecOneOffCommandParams) {
        ...
        let sandbox_type = match &effective_policy {
            tunacode_core::protocol::SandboxPolicy::DangerFullAccess => {
                tunacode_core::exec::SandboxType::None
            }
            _ => get_platform_sandbox().unwrap_or(tunacode_core::exec::SandboxType::None),
        };
        tracing::debug!("Sandbox type: {sandbox_type:?}");
        let tunacode_linux_sandbox_exe = self.config.tunacode_linux_sandbox_exe.clone();
        let outgoing = self.outgoing.clone();
        let req_id = request_id;
        let sandbox_cwd = self.config.cwd.clone();

        tokio::spawn(async move {
            match tunacode_core::exec::process_exec_tool_call(
                exec_params,
                sandbox_type,
                &effective_policy,
                sandbox_cwd.as_path(),
    ...
    async fn handle_resume_conversation(
        ...
        let config = match params.overrides {
            Some(overrides) => {
                derive_config_from_params(overrides, self.tunacode_linux_sandbox_exe.clone())
            }
            ...
            .await
        {
            Ok(NewConversation {
                ...
                let response = tunacode_protocol::mcp_protocol::ResumeConversationResponse {
                    conversation_id,
                    model: session_configured.model.clone(),
                    initial_messages,
                };
    ...
    async fn archive_conversation(&self, request_id: RequestId, params: ArchiveConversationParams) {
        ...
        let result: std::io::Result<()> = async {
            let archive_folder = self
                .config
                .tunacode_home
                .join(tunacode_core::ARCHIVED_SESSIONS_SUBDIR);
            ...
            loop {
                tokio::select! {
                    ...
                    event = conversation.next_event() => {
                        ...
                        // instead.
                        let method = format!("tunacode/event/{}", event.msg);
...
async fn apply_bespoke_event_handling(
    ...
    conversation: Arc<tunacodeConversation>,
    outgoing: Arc<OutgoingMessageSender>,
    pending_interrupts: Arc<Mutex<HashMap<ConversationId, Vec<RequestId>>>>,
) {
...
async fn on_patch_approval_response(
    ...
    tunacode: Arc<tunacodeConversation>,
) {
    ...
    let value = match response {
        ...
        Err(err) => {
            ...
            if let Err(submit_err) = tunacode
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `tunacode`.
    let response =
