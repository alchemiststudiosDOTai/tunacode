...
async fn logout_chatgpt_removes_auth() {
    ...
    login_with_api_key(tunacode_home.path(), "sk-test-key").expect("seed api key");
    assert!(tunacode_home.path().join("auth.json").exists());
    ...
    assert!(
        !tunacode_home.path().join("auth.json").exists(),
...
async fn login_and_cancel_chatgpt() {
    ...
    let maybe_note = timeout(
        ...
        mcp.read_stream_until_notification_message("tunacode/event/login_chat_gpt_complete"),
    )
    .await;
