...
async fn test_tunacode_jsonrpc_conversation_flow() {
    ...
    // Temporary tunacode home with config pointing at the mock server.
    let tunacode_home = tmp.path().join("tunacode_home");
    std::fs::create_dir(&tunacode_home).expect("create tunacode home dir");
    ...
    create_config_toml(&tunacode_home, &server.uri()).expect("write config");

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&tunacode_home).await.expect("spawn mcp");
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![tunacode_protocol::mcp_protocol::InputItem::Text {
                text: "text".to_string(),
            }],
        })
        .await
    ...
    let task_finished_notification: JSONRPCNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("tunacode/event/task_complete"),
    )
    .await
...
async fn test_send_user_turn_changes_approval_policy_behavior() {
    ...
    mcp.send_response(
        request.id,
        serde_json::json!({ "decision": tunacode_core::protocol::ReviewDecision::Approved }),
    )
    .await
