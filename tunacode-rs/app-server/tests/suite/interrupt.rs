...
async fn shell_command_interruption() -> anyhow::Result<()> {
    ...
    std::fs::create_dir(&tunacode_home)?;
    ...
    .await;
    create_config_toml(&tunacode_home, server.uri())?;

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&tunacode_home).await?;
...
fn create_config_toml(tunacode_home: &Path, server_uri: String) -> std::io::Result<()> {
