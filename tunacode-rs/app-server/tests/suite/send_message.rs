...
async fn test_send_message_success() {
    // Spin up a mock completions server that immediately ends the tunacode turn.
    // Two tunacode turns hit the mock model (session start + send-user-message). Provide two SSE responses.
    let responses = vec![
    ...
    // Create a temporary tunacode home with config pointing at the mock server.
    let tunacode_home = TempDir::new().expect("create temp dir");
    create_config_toml(tunacode_home.path(), &server.uri()).expect("write config.toml");
...
async fn test_send_message_session_not_found() {
    // Start MCP without creating a tunacode session
    let tunacode_home = TempDir::new().expect("tempdir");
    let mut mcp = McpProcess::new(tunacode_home.path()).await.expect("spawn");
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize())
        .await
        .expect("timeout")
        .expect("init");
