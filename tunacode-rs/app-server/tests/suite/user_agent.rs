...
async fn get_user_agent_returns_current_tunacode_user_agent() {
    let tunacode_home = TempDir::new().unwrap_or_else(|err| panic!("create tempdir: {err}"));
    ...
    let user_agent = format!(
        "tunacode_cli_rs/0.0.0 ({} {}; {}) {} (tunacode-app-server-tests; 0.1.0)",
        os_info.os_type(),
        os_info.version(),
