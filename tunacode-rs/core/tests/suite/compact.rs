...
async fn summarize_context_three_requests_and_instructions() {
    ...
    tunacode.submit(Op::Compact).await.unwrap();
    wait_for_event(&tunacode, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
    ...
    // Shut down tunacode to flush rollout entries before inspecting the file.
    tunacode.submit(Op::Shutdown).await.unwrap();
...
async fn auto_compact_stops_after_failed_attempt() {
    ...
    let error_event = wait_for_event(&tunacode, |ev| matches!(ev, EventMsg::Error(_))).await;
...
async fn auto_compact_allows_multiple_attempts_when_interleaved_with_other_turn_events() {
    ...
    loop {
        let event = tunacode.next_event().await.unwrap();
