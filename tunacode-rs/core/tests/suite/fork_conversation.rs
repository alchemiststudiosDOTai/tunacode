...
use tunacode_core::content_items_to_text;
use tunacode_core::is_session_prefix_message;
...
async fn fork_conversation_twice_drops_to_first_message() {
    ...
    // Configure tunacode to use the mock server.
    ...
    for text in ["first", "second", "third"] {
        tunacode
            ...
            .await
            .unwrap();
        let _ = wait_for_event(&tunacode, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
    }
    ...
    tunacode_fork1.submit(Op::GetPath).await.unwrap();
    let fork1_history = wait_for_event(&tunacode_fork1, |ev| {
    ...
    tunacode_fork2.submit(Op::GetPath).await.unwrap();
    let fork2_history = wait_for_event(&tunacode_fork2, |ev| {
        matches!(ev, EventMsg::ConversationPath(_))
    })
    .await;
