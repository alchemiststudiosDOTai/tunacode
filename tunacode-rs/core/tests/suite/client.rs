...
use tunacode_core::LocalShellStatus;
use tunacode_core::ModelClient;
use tunacode_core::ModelProviderInfo;
use tunacode_core::NewConversation;
use tunacode_core::Prompt;
use tunacode_core::ReasoningItemContent;
use tunacode_core::ResponseEvent;
use tunacode_core::ResponseItem;
...
/// Writes an `auth.json` into the provided `tunacode_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        tunacode_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = tunacode_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![tunacode_protocol::models::ContentItem::InputText {
    ...
    let prior_system = tunacode_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![tunacode_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = tunacode_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure tunacode to resume from our file
    let model_provider = ModelProviderInfo {
        base_url: Some(format!("{}/v1", server.uri())),
        ..built_in_model_providers()["openai"].clone()
    };
    let tunacode_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&tunacode_home);
    ...
    tunacode
        ...
        .await
        .unwrap();
    wait_for_event(&tunacode, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "tunacode_cli_rs");
    assert_eq!(
...
async fn includes_base_instructions_override_in_request() {
    ...
    let conversation_manager =
        ConversationManager::with_auth(tunacodeAuth::from_api_key("Test API Key"));
    let tunacode = conversation_manager
        .new_conversation(config)
        .await
        .expect("create new conversation")
        .conversation;
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    Mock::given(method("POST"))
        .and(path("/api/tunacode/responses"))
        .respond_with(first)
        .expect(1)
        .mount(&server)
        .await;
    ...
    let model_provider = ModelProviderInfo {
        base_url: Some(format!("{}/api/tunacode", server.uri())),
        ..built_in_model_providers()["openai"].clone()
    };
    ...
    let mut config = load_default_config_for_test(&tunacode_home);
    config.model_provider = model_provider;
    let conversation_manager = ConversationManager::with_auth(create_dummy_tunacode_auth());
    let NewConversation {
        conversation: tunacode,
        conversation_id,
        session_configured: _,
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match tunacodeAuth::from_tunacode_home(tunacode_home.path()) {
        Ok(Some(auth)) => tunacode_core::AuthManager::from_auth_for_testing(auth),
        Ok(None) => panic!("No tunacodeAuth found in tunacode_home"),
        Err(e) => panic!("Failed to load tunacodeAuth: {e}"),
    };
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-tunacode-primary-used-percent", "12.5")
        .insert_header("x-tunacode-secondary-used-percent", "40.0")
        .insert_header("x-tunacode-primary-window-minutes", "10")
        .insert_header("x-tunacode-secondary-window-minutes", "60")
        .insert_header("x-tunacode-primary-reset-after-seconds", "1800")
        .insert_header("x-tunacode-secondary-reset-after-seconds", "7200")
        .set_body_raw(sse_body, "text/event-stream");
    ...
    let conversation_manager = ConversationManager::with_auth(tunacodeAuth::from_api_key("test"));
    let tunacode = conversation_manager
        .new_conversation(config)
        .await
    ...
    let first_token_event =
        wait_for_event(&tunacode, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5-tunacode in tests → 272000 context window
    ...
    wait_for_event(&tunacode, |msg| matches!(msg, EventMsg::TaskComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-tunacode-primary-used-percent", "100.0")
        .insert_header("x-tunacode-secondary-used-percent", "87.5")
        .insert_header("x-tunacode-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-tunacode-primary-window-minutes", "15")
        .insert_header("x-tunacode-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_tunacode();
    let tunacode_fixture = builder.build(&server).await?;
    let tunacode = tunacode_fixture.tunacode.clone();
    ...
    let token_event = wait_for_event(&tunacode, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&tunacode, |msg| matches!(msg, EventMsg::Error(_))).await;
...
fn create_dummy_tunacode_auth() -> tunacodeAuth {
    tunacodeAuth::create_dummy_chatgpt_auth_for_testing()
}
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under tunacode sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
    ...
    // Init session with isolated tunacode home.
    let tunacode_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&tunacode_home);
    config.model_provider = model_provider;
