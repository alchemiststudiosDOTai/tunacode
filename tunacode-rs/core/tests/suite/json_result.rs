...
async fn tunacode_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    tunacode_returns_json_result("gpt-5".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn tunacode_returns_json_result_for_gpt5_tunacode() -> anyhow::Result<()> {
    tunacode_returns_json_result("gpt-5-tunacode".to_string()).await
}

async fn tunacode_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("tunacode_output_schema".into()))
    ...
    let Testtunacode { tunacode, cwd, .. } = test_tunacode().build(&server).await?;
    ...
    let message = wait_for_event(&tunacode, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
