...
use tunacode_core::REVIEW_PROMPT;
...
use tunacode_core::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under tunacode sandbox network restrictions.
    skip_if_no_network!();
    ...
    let tunacode = new_conversation_for_server(&server, &tunacode_home, |_| {}).await;
    ...
    let _complete = wait_for_event(&tunacode, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    tunacode.submit(Op::GetPath).await.unwrap();
    let history_event =
        wait_for_event(&tunacode, |ev| matches!(ev, EventMsg::ConversationPath(_))).await;
...
async fn review_does_not_emit_agent_message_on_structured_output() {
    ...
    loop {
        let ev = timeout(Duration::from_secs(5), tunacode.next_event())
            .await
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let tunacode = new_conversation_for_server(&server, &tunacode_home, |cfg| {
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = tunacode_home.path().join("resume.jsonl");
    {
        ...
        let user = tunacode_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = tunacode_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let tunacode =
        resume_conversation_for_server(&server, &tunacode_home, session_file.clone(), |_| {}).await;
