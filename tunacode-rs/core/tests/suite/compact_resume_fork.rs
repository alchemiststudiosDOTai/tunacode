...
//! request payload that tunacode would send to the model and assert that the
...
async fn start_test_conversation(
    ...
    let manager = ConversationManager::with_auth(tunacodeAuth::from_api_key("dummy"));
...
async fn user_turn(conversation: &Arc<tunacodeConversation>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<tunacodeConversation>) {
    conversation
        .submit(Op::Compact)
        .await
...
async fn resume_conversation(
    ...
    let auth_manager =
        tunacode_core::AuthManager::from_auth_for_testing(tunacodeAuth::from_api_key("dummy"));
    let NewConversation { conversation, .. } = manager
