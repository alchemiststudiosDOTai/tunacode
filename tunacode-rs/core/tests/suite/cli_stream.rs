...
/// 2. Configures tunacode to use this mock server via a custom provider
...
async fn chat_mode_stream_cli() {
    ...
    cmd.arg("run")
        ...
        .arg("hello?");
    cmd.env("tunacode_HOME", home.path())
...
async fn exec_cli_applies_experimental_instructions_file() {
    ...
    // tunacode to use the Responses API with the dummy env var.
    let provider_override = format!(
...
/// 2. Configures tunacode to read from this fixture via tunacode_RS_SSE_FIXTURE env var
...
async fn responses_api_stream_cli() {
    ...
    cmd.env("tunacode_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("tunacode_RS_SSE_FIXTURE", fixture)
...
async fn integration_creates_and_checks_session_file() {
    ...
    // 4. Run the tunacode CLI through cargo (ensures the right bin is built) and invoke `exec`,
    ...
    cmd2.arg("run")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("tunacode_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("tunacode_RS_SSE_FIXTURE", &fixture)
        .env("OPENAI_BASE_URL", "http://unused.local");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume tunacode-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = tunacode_core::git_info::collect_git_info(&git_repo).await;
