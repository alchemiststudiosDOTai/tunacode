...
async fn responses_api_emits_api_request_event() {
    ...
    let Testtunacode { tunacode, .. } = test_tunacode().build(&server).await.unwrap();
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("tunacode.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected tunacode.api_request event".to_string()))
    });

    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("tunacode.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected tunacode.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("tunacode.sse_event")
...
async fn process_sse_emits_failed_event_on_parse_error() {
    ...
    let Testtunacode { tunacode, .. } = test_tunacode()
    ...
    logs_assert(|lines: &[&str]| {
        lines
            ...
            .map(|_| Ok(()))
            .unwrap_or(Err("missing tunacode.sse_event".to_string()))
    });
}
...
async fn process_sse_failed_event_logs_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("tunacode.sse_event") && line.contains("event.kind=response.failed")
            })
            .map(|_| Ok(()))
            .unwrap_or(Err("missing tunacode.sse_event".to_string()))
    });
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("tunacode.tool_result") && line.contains("call_id=custom-tool-call")
            })
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("tunacode.tool_result") && line.contains("call_id=function-call")
            })
            .ok_or_else(|| "missing tunacode.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_local_shell_missing_ids() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("tunacode.tool_result")
