...
fn network_disabled() -> bool {
    std::env::var(TUNACODE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn run_stream_with_bytes(sse_body: &[u8]) -> Vec<ResponseEvent> {
    ...
    let tunacode_home = match TempDir::new() {
        Ok(dir) => dir,
...
fn assert_reasoning(item: &ResponseItem, expected: &str) {
    ...
    } = item
    {
        ...
        for part in parts {
            match part {
                tunacode_core::ReasoningItemContent::ReasoningText { text }
                | tunacode_core::ReasoningItemContent::Text { text } => combined.push_str(text),
            }
        }
...
async fn chat_sse_done_chunk_emits_event() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("tunacode.sse_event") && line.contains("event.kind=message"))
            .map(|_| Ok(()))
            .unwrap_or(Err("cannot find SSE event".to_string()))
    });
}
