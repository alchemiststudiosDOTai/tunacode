...
pub mod test_tunacode_exec;

/// Returns a default `Config` whose on-disk state is confined to the provided
/// temporary directory. Using a per-test directory keeps tests hermetic and
/// avoids clobbering a developer’s real `~/.tunacode`.
pub fn load_default_config_for_test(tunacode_home: &TempDir) -> Config {
...
pub async fn wait_for_event<F>(
    tunacode: &tunacodeConversation,
    predicate: F,
) -> tunacode_core::protocol::EventMsg
where
    F: FnMut(&tunacode_core::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(tunacode, predicate, Duration::from_secs(1)).await
}
    ...
    F: FnMut(&tunacode_core::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(5)), tunacode.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    tunacode_core::spawn::TUNACODE_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    tunacode_core::spawn::TUNACODE_SANDBOX_NETWORK_DISABLED_ENV_VAR
}
