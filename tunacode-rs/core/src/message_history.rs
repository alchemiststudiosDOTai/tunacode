...
//!
//! The history is stored at `~/.tunacode/history.jsonl` with **one JSON object per
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.tunacode_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.tunacode/history.jsonl` and ensure the parent directory exists.
