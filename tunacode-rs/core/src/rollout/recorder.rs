//! Persist tunacode session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use tunacode_protocol::protocol::ResumedHistory;
use tunacode_protocol::protocol::RolloutItem;
use tunacode_protocol::protocol::RolloutLine;
use tunacode_protocol::protocol::SessionMeta;
use tunacode_protocol::protocol::SessionMetaLine;
...
///
/// ```ignore
/// $ jq -C . ~/.tunacode/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.tunacode/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List conversations (rollout files) under the provided tunacode home directory.
    pub async fn list_conversations(
        tunacode_home: &Path,
        page_size: usize,
        cursor: Option<&Cursor>,
    ) -> std::io::Result<ConversationsPage> {
        get_conversations(tunacode_home, page_size, cursor).await
    }
...
fn create_log_file(
    ...
    // Resolve ~/.tunacode/sessions/YYYY/MM/DD and create it if missing.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.tunacode_home.clone();
