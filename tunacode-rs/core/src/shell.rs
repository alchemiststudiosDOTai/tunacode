        ...
        match self {
            ...
            Shell::PowerShell(ps) => {
                ...
                if first != Some(ps.exe.as_str()) {
                    // TODO (tunacode_2900): Handle escaping newlines.
...
mod tests_windows {
    ...
    fn test_format_default_shell_invocation_powershell() {
        let cases = vec![
            ...
            (
                // TODO (tunacode_2900): Handle escaping newlines for powershell invocation.
                Shell::PowerShell(PowerShellConfig {
                    exe: "powershell.exe".to_string(),
                    bash_exe_fallback: Some(PathBuf::from("bash.exe")),
                }),
                vec![
                    "tunacode-mcp-server.exe",
                    "--tunacode-run-as-apply-patch",
