...
pub enum tunacodeErr {
    ...
    /// could not be captured). Analogous to the previous `tunacodeError::Spawn` variant.
    ...
    #[error(
        "To use tunacode with your ChatGPT plan, upgrade to Plus: https://openai.com/chatgpt/pricing."
    )]
    UsageNotIncluded,
    ...
    #[error("tunacode-linux-sandbox was required but not provided")]
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let message = match self.plan_type.as_ref() {
            ...
            Some(PlanType::Known(KnownPlan::Free)) => {
                "You've hit your usage limit. Upgrade to Plus to continue using tunacode (https://openai.com/chatgpt/pricing)."
                    .to_string()
            }
...
impl tunacodeErr {
    /// Minimal shim so that existing `e.downcast_ref::<tunacodeErr>()` checks continue to compile
...
pub fn get_error_message_ui(e: &tunacodeErr) -> String {
    match e {
        tunacodeErr::Sandbox(SandboxErr::Denied { output }) => output.stderr.text.clone(),
        // Timeouts are not sandbox errors from a UX perspective; present them plainly
        tunacodeErr::Sandbox(SandboxErr::Timeout { output }) => format!(
...
mod tests {
    use super::*;
    use tunacode_protocol::protocol::RateLimitWindow;
