...
use tunacode_protocol::config_types::Verbosity;
use tunacode_protocol::mcp_protocol::Tools;
use tunacode_protocol::mcp_protocol::UserSavedConfig;
use dirs::home_dir;
...
const OPENAI_DEFAULT_MODEL: &str = "gpt-5-tunacode";
const OPENAI_DEFAULT_REVIEW_MODEL: &str = "gpt-5-tunacode";
pub const GPT_5_tunacode_MEDIUM_MODEL: &str = "gpt-5-tunacode";
...
pub struct Config {
    ...
    /// Model used specifically for review sessions. Defaults to "gpt-5-tunacode".
    ...
    /// Optional external notifier command. When set, tunacode will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - tunacode
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.tunacode/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "tunacode"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send tunacode '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// Definition for MCP servers that tunacode can reach out to for tool calls.
    ...
    /// Directory containing all tunacode state (defaults to `~/.tunacode` but can be
    /// overridden by the `tunacode_HOME` environment variable).
    pub tunacode_home: PathBuf,

    /// Settings that govern if and what will be written to `~/.tunacode/history.jsonl`.
    pub history: History,
    ...
    /// Path to the `tunacode-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `tunacode-linux-sandbox`.
    pub tunacode_linux_sandbox_exe: Option<PathBuf>,
...
impl Config {
    ...
    pub fn load_with_cli_overrides(
        ...
        // Resolve the directory that stores tunacode state (e.g. ~/.tunacode or the
        // value of $tunacode_HOME) so we can embed it into the resulting
        // `Config` instance.
        let tunacode_home = find_tunacode_home()?;

        // Step 1: parse `config.toml` into a generic JSON value.
        let mut root_value = load_config_as_toml(&tunacode_home)?;
        ...
        Self::load_from_base_config_with_overrides(cfg, overrides, tunacode_home)
    }
}
...
/// Read `tunacode_HOME/config.toml` and return it as a generic TOML value. Returns
/// an empty TOML table when the file does not exist.
pub fn load_config_as_toml(tunacode_home: &Path) -> std::io::Result<TomlValue> {
    let config_path = tunacode_home.join(CONFIG_TOML_FILE);
...
pub fn load_global_mcp_servers(
    ...
    let root_value = load_config_as_toml(tunacode_home)?;
...
pub fn write_global_mcp_servers(
    ...
    std::fs::create_dir_all(tunacode_home)?;
    let tmp_file = NamedTempFile::new_in(tunacode_home)?;
...
/// Patch `tunacode_HOME/config.toml` project state.
/// Use with caution.
pub fn set_project_trusted(tunacode_home: &Path, project_path: &Path) -> anyhow::Result<()> {
    let config_path = tunacode_home.join(CONFIG_TOML_FILE);
...
pub async fn persist_model_selection(
    ...
    tokio::fs::create_dir_all(tunacode_home)
        .await
        .with_context(|| {
            format!(
                "failed to create tunacode home directory at {}",
...
/// Base config deserialized from ~/.tunacode/config.toml.
...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        cfg: ConfigToml,
        overrides: ConfigOverrides,
        tunacode_home: PathBuf,
    ) -> std::io::Result<Self> {
        let user_instructions = Self::load_instructions(Some(&tunacode_home));
    ...
    fn load_instructions(tunacode_dir: Option<&Path>) -> Option<String> {
        let mut p = match tunacode_dir {
...
/// Returns the path to the tunacode configuration directory, which can be
/// specified by the `tunacode_HOME` environment variable. If not set, defaults to
/// `~/.tunacode`.
///
/// - If `tunacode_HOME` is set, the value will be canonicalized and this
///   function will Err if the path does not exist.
/// - If `tunacode_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_tunacode_home() -> std::io::Result<PathBuf> {
    // Honor the `tunacode_HOME` environment variable when it is set to allow users
    // (and tests) to override the default location.
    if let Ok(val) = std::env::var("tunacode_HOME")
        && !val.is_empty()
    {
...
/// Returns the path to the folder where tunacode logs are stored. Does not verify
/// that the directory exists.
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    let mut p = cfg.tunacode_home.clone();
    p.push("log");
    Ok(p)
}
...
"#;
    ...
    fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        let tunacode_home = TempDir::new()?;

        let servers = load_global_mcp_servers(tunacode_home.path())?;
    ...
    fn write_global_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        write_global_mcp_servers(tunacode_home.path(), &servers)?;

        let loaded = load_global_mcp_servers(tunacode_home.path())?;
        ...
        write_global_mcp_servers(tunacode_home.path(), &empty)?;
        let loaded = load_global_mcp_servers(tunacode_home.path())?;
    ...
    fn load_global_mcp_servers_accepts_legacy_ms_field() -> anyhow::Result<()> {
        let tunacode_home = TempDir::new()?;
        let config_path = tunacode_home.path().join(CONFIG_TOML_FILE);
...
"#
    ...
    async fn persist_model_selection_updates_defaults() -> anyhow::Result<()> {
        ...
        let serialized =
            tokio::fs::read_to_string(tunacode_home.path().join(CONFIG_TOML_FILE)).await?;
        let parsed: ConfigToml = toml::from_str(&serialized)?;

        assert_eq!(parsed.model.as_deref(), Some("gpt-5-tunacode"));
...
"#,
    ...
    async fn persist_model_selection_updates_profile() -> anyhow::Result<()> {
        ...
        assert_eq!(profile.model.as_deref(), Some("gpt-5-tunacode"));
        assert_eq!(
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn tunacode_home(&self) -> PathBuf {
            self.tunacode_home.path().to_path_buf()
        }
    }
...
"#;
        ...
        let tunacode_home_temp_dir = TempDir::new().unwrap();
        ...
        Ok(PrecedenceTestFixture {
            cwd: cwd_temp_dir,
            tunacode_home: tunacode_home_temp_dir,
            cfg,
            model_provider_map,
            openai_provider,
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                tunacode_home: fixture.tunacode_home(),
                history: History::default(),
                file_opener: UriBasedFileOpener::VsCode,
                tunacode_linux_sandbox_exe: None,
...
projects = { "/Users/mbolin/code/tunacode4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/tunacode3" = { trust_level = "trusted" } }
model = "foo""#;
        let mut doc = initial.parse::<DocumentMut>()?;

        // Approve a new directory
        let new_project = Path::new("/Users/mbolin/code/tunacode2");
...
[projects."/Users/mbolin/code/tunacode4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/tunacode3"]
trust_level = "trusted"

[projects."/Users/mbolin/code/tunacode2"]
trust_level = "trusted"
"#;
        assert_eq!(contents, expected);
