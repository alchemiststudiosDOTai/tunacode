...
impl From<ParsedCommand> for tunacode_protocol::parse_command::ParsedCommand {
    fn from(v: ParsedCommand) -> Self {
        use tunacode_protocol::parse_command::ParsedCommand as P;
        match v {
...
/// The easiest way to iterate is to add unit tests and have tunacode fix the implementation.
...
/// Tests are at the top to encourage using TDD + tunacode to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_grep_recursive_current_dir() {
        assert_parsed(
            &vec_str(&["grep", "-R", "TUNACODE_SANDBOX_ENV_VAR", "-n", "."]),
            vec![ParsedCommand::Search {
                cmd: "grep -R TUNACODE_SANDBOX_ENV_VAR -n .".to_string(),
                query: Some("TUNACODE_SANDBOX_ENV_VAR".to_string()),
                path: Some(".".to_string()),
            }],
        );
    }
    ...
    fn supports_grep_recursive_specific_file() {
        assert_parsed(
            &vec_str(&[
                "grep",
                "-R",
                "TUNACODE_SANDBOX_ENV_VAR",
                "-n",
                "core/src/spawn.rs",
            ]),
            vec![ParsedCommand::Search {
                cmd: "grep -R TUNACODE_SANDBOX_ENV_VAR -n core/src/spawn.rs".to_string(),
                query: Some("TUNACODE_SANDBOX_ENV_VAR".to_string()),
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd tunacode-rs && rg --files"),
            vec![ParsedCommand::Search {
                cmd: "rg --files".to_string(),
                query: None,
                path: None,
