...
impl ModelClient {
    ...
    async fn stream_responses(&self, prompt: &Prompt) -> Result<ResponseStream> {
        if let Some(path) = &*tunacode_RS_SSE_FIXTURE {
    ...
    async fn attempt_stream_responses(
        ...
        match res {
            Ok(resp) if resp.status().is_success() => {
                ...
                let stream = resp.bytes_stream().map_err(tunacodeErr::Reqwest);
            ...
            Ok(res) => {
                    ...
                    || status.is_server_error())
                {
                    ...
                    return Err(StreamAttemptError::Fatal(tunacodeErr::UnexpectedStatus(
                        status, body,
                    )));
                }
...
enum StreamAttemptError {
    ...
    RetryableTransportError(tunacodeErr),
    Fatal(tunacodeErr),
}
...
impl StreamAttemptError {
    ...
    fn into_error(self) -> tunacodeErr {
        match self {
            Self::RetryableHttpError { status, .. } => {
                if status == StatusCode::INTERNAL_SERVER_ERROR {
                    tunacodeErr::InternalServerError
                } else {
                    tunacodeErr::RetryLimit(status)
                }
            }
...
fn parse_rate_limit_snapshot(headers: &HeaderMap) -> Option<RateLimitSnapshot> {
    let primary = parse_rate_limit_window(
        headers,
        "x-tunacode-primary-used-percent",
        "x-tunacode-primary-window-minutes",
        "x-tunacode-primary-reset-after-seconds",
    );

    let secondary = parse_rate_limit_window(
        headers,
        "x-tunacode-secondary-used-percent",
        "x-tunacode-secondary-window-minutes",
        "x-tunacode-secondary-reset-after-seconds",
    );
    ...
    S: Stream<Item = Result<Bytes>> + Unpin,
{
    ...
    let mut response_error: Option<tunacodeErr> = None;
    ...
    loop {
            ...
            .await
        {
            ...
            Ok(Some(Err(e))) => {
                debug!("SSE Error: {e:#}");
                let event = tunacodeErr::Stream(e.to_string(), None);
            ...
            Ok(None) => {
                match response_completed {
                    ...
                    None => {
                        let error = response_error.unwrap_or(tunacodeErr::Stream(
        ...
        match event.kind.as_str() {
            ...
            "response.failed" => {
                if let Some(resp_val) = event.response {
                    response_error = Some(tunacodeErr::Stream(
                    ...
                    if let Some(error) = error {
                        match serde_json::from_value::<Error>(error.clone()) {
                            Ok(error) => {
                                ...
                                response_error = Some(tunacodeErr::Stream(message, delay));
                            }
                            Err(e) => {
                                let error = format!("failed to parse ErrorResponse: {e}");
                                debug!(error);
                                response_error = Some(tunacodeErr::Stream(error, None))
                            }
                        }
                    }
                }
...
async fn stream_from_fixture(
    ...
    let stream = ReaderStream::new(rdr).map_err(tunacodeErr::Io);
    tokio::spawn(process_sse(
        stream,
        tx_event,
...
mod tests {
    ...
    async fn collect_events(
        ...
        let stream = ReaderStream::new(reader).map_err(tunacodeErr::Io);
    ...
    async fn run_sse(
        ...
        let stream = ReaderStream::new(std::io::Cursor::new(body)).map_err(tunacodeErr::Io);
        tokio::spawn(process_sse(
            stream,
            tx,
            provider.stream_idle_timeout(),
    ...
    async fn error_when_error_event() {
        ...
        match &events[0] {
            Err(tunacodeErr::Stream(msg, delay)) => {
                assert_eq!(
                    msg,
