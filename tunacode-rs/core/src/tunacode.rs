...
use tunacode_apply_patch::MaybeApplyPatchVerified;
use tunacode_apply_patch::maybe_parse_apply_patch_verified;
use tunacode_protocol::mcp_protocol::ConversationId;
use tunacode_protocol::protocol::ConversationPathResponseEvent;
use tunacode_protocol::protocol::ExitedReviewModeEvent;
use tunacode_protocol::protocol::ReviewRequest;
use tunacode_protocol::protocol::RolloutItem;
use tunacode_protocol::protocol::TaskStartedEvent;
use tunacode_protocol::protocol::TurnAbortReason;
use tunacode_protocol::protocol::TurnContextItem;
use futures::prelude::*;
...
use crate::apply_patch::tunacode_APPLY_PATCH_ARG1;
...
use tunacode_otel::otel_event_manager::ToolDecisionSource;
...
use tunacode_protocol::models::LocalShellAction;
use tunacode_protocol::models::ResponseInputItem;
use tunacode_protocol::models::ResponseItem;
use tunacode_protocol::models::ShellToolCallParams;
...
/// The high-level interface to the tunacode system.
...
/// Wrapper returned by [`tunacode::spawn`] containing the spawned [`tunacode`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct tunacodeSpawnOk {
    pub tunacode: tunacode,
...
impl tunacode {
    /// Spawn a new [`tunacode`] and initialize the session.
    pub async fn spawn(
        config: Config,
        auth_manager: Arc<AuthManager>,
        conversation_history: InitialHistory,
    ) -> tunacodeResult<tunacodeSpawnOk> {
        ...
        // Generate a unique ID for the lifetime of this tunacode session.
    ...
    /// Use sparingly: prefer `submit()` so tunacode is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, sub: Submission) -> tunacodeResult<()> {
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| tunacodeErr::InternalAgentDied)?;
        Ok(())
    }
...
impl Session {
    async fn new(
        ...
        let services = SessionServices {
            ...
            tunacode_linux_sandbox_exe: config.tunacode_linux_sandbox_exe.clone(),
    ...
    async fn run_exec_with_events<'a>(
        ...
        let result = process_exec_tool_call(
            ...
            exec_args.tunacode_linux_sandbox_exe,
    ...
    while let Ok(sub) = rx_sub.recv().await {
        ...
        match sub.op {
                        ...
                        msg: EventMsg::GetHistoryEntryResponse(
                            crate::protocol::GetHistoryEntryResponseEvent {
                                ...
                                entry: entry_opt.map(|e| {
                                    tunacode_protocol::message_history::HistoryEntry {
            ...
            Op::Shutdown => {
                ...
                info!("Shutting down tunacode instance");
...
pub(crate) async fn run_task(
    ...
    // Although from the perspective of tunacode.rs, TurnDiffTracker has the lifecycle of a Task which contains
...
async fn run_turn(
    ...
    loop {
        match try_run_turn(sess, turn_context, turn_diff_tracker, &sub_id, &prompt).await {
            ...
            Err(tunacodeErr::Interrupted) => return Err(tunacodeErr::Interrupted),
            Err(tunacodeErr::EnvVar(var)) => return Err(tunacodeErr::EnvVar(var)),
            Err(tunacodeErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&sub_id, rate_limits).await;
                }
                return Err(tunacodeErr::UsageLimitReached(e));
            }
            Err(tunacodeErr::UsageNotIncluded) => return Err(tunacodeErr::UsageNotIncluded),
            Err(e) => {
                ...
                if retries < max_retries {
                    retries += 1;
                    let delay = match e {
                        tunacodeErr::Stream(_, Some(delay)) => delay,
                        _ => backoff(retries),
                    };
                    warn!(
...
async fn handle_response_item(
    ...
    sub_id: &str,
    item: ResponseItem,
) -> tunacodeResult<Option<ResponseInputItem>> {
...
pub struct ExecInvokeArgs<'a> {
    ...
    pub tunacode_linux_sandbox_exe: &'a Option<PathBuf>,
...
async fn handle_container_exec_with_params(
    ...
    let (params, safety, command_for_display) = match &apply_patch_exec {
        Some(ApplyPatchExec {
            ...
            let path_to_tunacode = std::env::current_exe()
                .ok()
                .map(|p| p.to_string_lossy().to_string());
            let Some(path_to_tunacode) = path_to_tunacode else {
                return Err(FunctionCallError::RespondToModel(
                    "failed to determine path to tunacode executable".to_string(),
                ));
            };
            ...
            let params = ExecParams {
                command: vec![
                    path_to_tunacode,
                    tunacode_APPLY_PATCH_ARG1.to_string(),
                    patch.clone(),
                ],
                cwd: cwd.clone(),
                timeout_ms: params.timeout_ms,
    ...
    let output_result = sess
        .run_exec_with_events(
            ...
            ExecInvokeArgs {
                ...
                tunacode_linux_sandbox_exe: &sess.services.tunacode_linux_sandbox_exe,
...
mod tests {
    ...
    pub(crate) fn make_session_and_context() -> (Session, TurnContext) {
        ...
        let tunacode_home = tempfile::tempdir().expect("create temp dir");
        let config = Config::load_from_base_config_with_overrides(
            ConfigToml::default(),
            ConfigOverrides::default(),
            tunacode_home.path().to_path_buf(),
        )
