...
pub(crate) async fn stream_chat_completions(
    ...
    if prompt.output_schema.is_some() {
        return Err(tunacodeErr::UnsupportedOperation(
        ...
        match res {
            ...
            Ok(res) => {
                ...
                if !(status == StatusCode::TOO_MANY_REQUESTS || status.is_server_error()) {
                    ...
                    return Err(tunacodeErr::UnexpectedStatus(status, body));
                }

                if attempt > max_retries {
                    return Err(tunacodeErr::RetryLimit(status));
                }
...
/// output is mapped onto tunacode's internal [`ResponseEvent`] so that the rest
    ...
    S: Stream<Item = Result<Bytes>> + Unpin,
{
    ...
    loop {
            ...
            .await
        {
            ...
            Ok(Some(Err(e))) => {
                let _ = tx_event
                    .send(Err(tunacodeErr::Stream(e.to_string(), None)))
                    .await;
                return;
            }
            Ok(None) => {
            ...
            match Pin::new(&mut this.inner).poll_next(cx) {
                ...
                Poll::Ready(Some(Ok(ResponseEvent::OutputItemDone(item)))) => {
                    ...
                    let is_assistant_message = matches!(
                        &item,
                        tunacode_protocol::models::ResponseItem::Message { role, .. } if role == "assistant"
                    );
                        ...
                        match this.mode {
                            AggregateMode::AggregatedOnly => {
                                ...
                                if this.cumulative.is_empty()
                                    && let tunacode_protocol::models::ResponseItem::Message {
                                        content,
                                        ..
                                    } = &item
                                    && let Some(text) = content.iter().find_map(|c| match c {
                                        tunacode_protocol::models::ContentItem::OutputText {
                                            text,
                                        } => Some(text),
                                        _ => None,
                                    })
                ...
                Poll::Ready(Some(Ok(ResponseEvent::Completed {
                        ...
                        && matches!(this.mode, AggregateMode::AggregatedOnly)
                    {
                        let aggregated_reasoning =
                            tunacode_protocol::models::ResponseItem::Reasoning {
                                id: String::new(),
                                summary: Vec::new(),
                                content: Some(vec![
                                    tunacode_protocol::models::ReasoningItemContent::ReasoningText {
                    ...
                    if !this.cumulative.is_empty() {
                        let aggregated_message = tunacode_protocol::models::ResponseItem::Message {
                            id: None,
                            role: "assistant".to_string(),
