...
/// See https://github.com/openai/tunacode/pull/3388/files for an example of what that would look like.
...
fn init_originator_from_env() -> Originator {
    let default = "tunacode_cli_rs";
    let value = std::env::var(tunacode_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
...
pub fn get_tunacode_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized tunacode user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base tunacode user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default tunacode originator because base user agent string is invalid"
        );
...
pub fn create_client() -> reqwest::Client {
    ...
    let ua = get_tunacode_user_agent();
...
fn is_sandboxed() -> bool {
    std::env::var(TUNACODE_SANDBOX_ENV_VAR).as_deref() == Ok("seatbelt")
}
...
mod tests {
    ...
    #[test]
    fn test_get_tunacode_user_agent() {
        let user_agent = get_tunacode_user_agent();
        assert!(user_agent.starts_with("tunacode_cli_rs/"));
    }
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        assert_eq!(originator_header.to_str().unwrap(), "tunacode_cli_rs");

        // User-Agent matches the computed tunacode UA for that originator
        let expected_ua = get_tunacode_user_agent();
    ...
    fn test_invalid_suffix_is_sanitized() {
        let prefix = "tunacode_cli_rs/0.0.0";
        let suffix = "bad\rsuffix";

        assert_eq!(
            sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
            "tunacode_cli_rs/0.0.0 (bad_suffix)"
        );
    }
    ...
    fn test_macos() {
        ...
        let user_agent = get_tunacode_user_agent();
        let re = Regex::new(
            r"^tunacode_cli_rs/\d+\.\d+\.\d+ \(Mac OS \d+\.\d+\.\d+; (x86_64|arm64)\) (\S+)$",
        )
        .unwrap();
