...
pub struct NewConversation {
    ...
    pub conversation: Arc<tunacodeConversation>,
...
pub struct ConversationManager {
    conversations: Arc<RwLock<HashMap<ConversationId, Arc<tunacodeConversation>>>>,
...
impl ConversationManager {
    ...
    /// Construct with a dummy AuthManager containing the provided tunacodeAuth.
    /// Used for integration tests: should not be used by ordinary business logic.
    pub fn with_auth(auth: tunacodeAuth) -> Self {
        Self::new(crate::AuthManager::from_auth_for_testing(auth))
    }

    pub async fn new_conversation(&self, config: Config) -> tunacodeResult<NewConversation> {
    ...
    async fn spawn_conversation(
        ...
        config: Config,
        auth_manager: Arc<AuthManager>,
    ) -> tunacodeResult<NewConversation> {
        let tunacodeSpawnOk {
            tunacode,
            conversation_id,
        } = tunacode::spawn(config, auth_manager, InitialHistory::New).await?;
        self.finalize_spawn(tunacode, conversation_id).await
    }
    ...
    async fn finalize_spawn(
        ...
        // history.
        let event = tunacode.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(tunacodeErr::SessionConfiguredNotFirstEvent);
            }
        };

        let conversation = Arc::new(tunacodeConversation::new(tunacode));
        self.conversations
            .write()
            .await
    ...
    pub async fn get_conversation(
        &self,
        conversation_id: ConversationId,
    ) -> tunacodeResult<Arc<tunacodeConversation>> {
        let conversations = self.conversations.read().await;
        conversations
            .get(&conversation_id)
            .cloned()
            .ok_or_else(|| tunacodeErr::ConversationNotFound(conversation_id))
    }
    ...
    pub async fn resume_conversation_from_rollout(
        ...
        let tunacodeSpawnOk {
            tunacode,
            conversation_id,
        } = tunacode::spawn(config, auth_manager, initial_history).await?;
        self.finalize_spawn(tunacode, conversation_id).await
    }

    /// Removes the conversation from the manager's internal map, though the
    /// conversation is stored as `Arc<tunacodeConversation>`, it is possible that
    ...
    pub async fn remove_conversation(
        &self,
        conversation_id: &ConversationId,
    ) -> Option<Arc<tunacodeConversation>> {
    ...
    pub async fn fork_conversation(
        ...
        let tunacodeSpawnOk {
            tunacode,
            conversation_id,
        } = tunacode::spawn(config, auth_manager, history).await?;
