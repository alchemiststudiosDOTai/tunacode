...
fn server_opts(tunacode_home: &tempfile::TempDir, issuer: String) -> ServerOptions {
    let mut opts = ServerOptions::new(tunacode_home.path().to_path_buf(), "client-id".to_string());
    opts.issuer = issuer;
...
async fn device_code_login_integration_succeeds() {
    ...
    let opts = server_opts(&tunacode_home, issuer);

    run_device_code_login(opts)
        .await
        .expect("device code login integration should succeed");

    let auth_path = get_auth_file(tunacode_home.path());
