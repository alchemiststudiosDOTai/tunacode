...
/// Applies the latest diff from a tunacode agent task.
#[derive(Debug, Parser)]
...
pub async fn run_apply_command(
    ...
    init_chatgpt_token_from_auth(&config.tunacode_home).await?;
...
async fn apply_diff(diff: &str, cwd: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let res = tunacode_git_apply::apply_git_patch(&req)?;
    if res.exit_code != 0 {
        anyhow::bail!(
