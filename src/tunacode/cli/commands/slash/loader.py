...
class SlashCommandLoader:
    ...
    def _build_directory_list(self) -> List[Tuple[Path, CommandSource, str]]:
        ...
        return [
            (self.project_root / ".tunacode" / "commands", CommandSource.PROJECT_TUNACODE, "project"),
            ...
            try:
                ...
                for cmd_name, cmd in dir_commands.items():
                    if cmd_name in all_commands:
                        ...
                        if source.value < existing_cmd._metadata.source.value if existing_cmd._metadata else float('inf'):
                ...
                stats["loaded_commands"] += len([c for c in dir_commands.values() if c.name in all_commands])
                        
            except Exception as e:
                errors.append((directory, e))
                logger.error(f"Error scanning {directory}: {e}")
                
        logger.info(f"Discovered {len(all_commands)} slash commands from {stats['scanned_dirs']} directories")
        return CommandDiscoveryResult(all_commands, conflicts, errors, stats)
    
    def _scan_directory(self, directory: Path, source: CommandSource, namespace: str) -> Dict[str, SlashCommand]:
