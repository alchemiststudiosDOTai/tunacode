...
class CommandValidator:
    ...
    def validate_shell_command(self, command: str) -> ValidationResult:
        ...
        if base_command not in self.ALLOWED_COMMANDS:
            violations.append(SecurityViolation(
                ...
                command=command,
                severity="warning" if self.security_level == SecurityLevel.PERMISSIVE else "error"
            ))
    ...
    def _check_file_access_patterns(self, command: str) -> List[SecurityViolation]:
        ...
        sensitive_patterns = [
            ...
            r'/home/[^/]+/\.ssh', r'~/.ssh',
            r'/var/log/', r'/proc/', r'/sys/',
            r'\.env', r'\.secret', r'\.key',
        ]
    ...
    def validate_glob_pattern(self, pattern: str) -> ValidationResult:
        ...
        dangerous_globs = [
            ...
            r'/etc/.*', r'/home/.*', r'/root/.*',  # System directories
            r'/var/.*', r'/usr/bin/.*', r'/bin/.*',
            r'.*\.key$', r'.*\.secret$', r'.*\.pem$',  # Sensitive files
        ]
