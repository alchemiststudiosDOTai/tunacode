...
class MarkdownTemplateProcessor:
    ...
    def process_template_with_context(self, content: str, args: List[str], 
                                    context: 'CommandContext') -> ContextInjectionResult:
    ...
    def _process_command_blocks_with_tracking(self, content: str, 
                                            context: 'CommandContext') -> Tuple[str, List[str]]:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if self._total_context_size > self.max_context_size:
                        self._warnings.append(f"Command output truncated due to size limit: {command}")
    ...
    def _process_file_inclusions_with_tracking(self, content: str, 
        ...
        def replace_file(match):
            ...
            try:
                base_path = Path(getattr(context.state_manager.config, 'current_directory', os.getcwd()))
                ...
                if full_path.exists() and full_path.is_file():
                    ...
                    if self._total_context_size + len(file_content) > self.max_context_size:
                        self._warnings.append(f"File content truncated due to size limit: {file_path}")
    ...
    def _process_glob_inclusions_with_tracking(self, content: str, 
                                             context: 'CommandContext') -> str:
        ...
        def replace_glob(match):
            ...
            try:
                ...
                if len(matching_files) > self.max_files - len(self._included_files):
                    self._warnings.append(f"Glob pattern matched too many files, truncating: {pattern}")
                ...
                for file_path in sorted(matching_files):
                    ...
                    try:
                        ...
                        # Check size limits
                        if self._total_context_size + len(file_content) > self.max_context_size:
                            self._warnings.append(f"Glob inclusion stopped due to size limit at: {file_path}")
                            break
