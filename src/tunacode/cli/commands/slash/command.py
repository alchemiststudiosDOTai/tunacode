...
class SlashCommand(Command):
    ...
    def _lazy_load(self) -> None:
        ...
        try:
            ...
            self._metadata = SlashCommandMetadata(
                description=frontmatter.get('description', 'Custom command') if frontmatter else 'Custom command',
    ...
    async def execute(self, args: 'CommandArgs', context: 'CommandContext') -> 'CommandResult':
        ...
        injection_result = processor.process_template_with_context(
            self._content, args, context
        )
        ...
        try:
            if context.state_manager.session.show_thoughts:
                from ....ui import console as ui
                await ui.muted(f"Context: {len(injection_result.included_files)} files, "
        ...
        if self._metadata and self._metadata.allowed_tools:
            ...
            if hasattr(context.state_manager, 'tool_handler') and context.state_manager.tool_handler:
                ...
                original_restrictions = getattr(context.state_manager.tool_handler, 'allowed_tools', None)
                context.state_manager.tool_handler.allowed_tools = self._metadata.allowed_tools
                
                try:
                    # Execute with restrictions
                    if context.process_request:
                        result = await context.process_request(injection_result.processed_content, context.state_manager, True)
                        return result
                    else:
